% (C) 2023 Zong-han, Xie.  All rights reserved.
% chktex-file 3
% chktex-file 8
% chktex-file 10
% chktex-file 13
...
\title{
%
Finite Difference Time Domain Simulation of Electrodynamics
%
}
...
Electrodynamics is widely applied in many aspects of human life including radio and optical communication, advanced digital processing and optical component in human life. Advancement in digital computing capabilities enables all kinds of numerical simulation.  Kane Yee from Lawrence Radiation Laboratory has published a paper on 1966 \citep{kane_yee_1966}. In this paper, the Maxwell's equations have been replaced by a set of finite difference equations with appropriately chosen field point locations. This notes describes the detail of implementation method. The library is implemented in modern C++ and provide Python interface for interactive computing and programming.
...
To model the propagation of electromagnetic waves, one needs to use Faraday's law and Ampere's law in Maxwell's equations.
\begin{eqnarray}
	...
	\label{e:maxwell:maxwell}
\end{eqnarray}


Consider inhomogeneous and isotropic materials, we have 
\begin{eqnarray}
	{\mathbf{D}(\mathbf{x},t)}&=&{\epsilon(\mathbf{x})}{\mathbf{E}(\mathbf{x},t)} \\
	{\mathbf{B}(\mathbf{x},t)}&=&{\mu(\mathbf{x})}{\mathbf{H}(\mathbf{x},t)}
	\label{e:maxwell:D_E_H_B}.
\end{eqnarray}

In Equation (\ref{e:maxwell:D_E_H_B}), $\mathbf{D}(\mathbf{x},t)$ is the electric displacement field; $\mathbf{B}(\mathbf{x},t)$ is the magnetic field; $\mathbf{E}(\mathbf{x},t)$ is the electric field; $\mathbf{H}(\mathbf{x},t)$ is magnetizing field; $\epsilon(\mathbf{x}$ is electric permittivity ; $\mu(\mathbf{x})$ is magnetic permeability.
